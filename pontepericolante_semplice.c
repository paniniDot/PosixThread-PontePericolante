...
int ponteLibero = 0; /* 0 se libero, 1 se pieno */
int autoInAttesa[2];
...
pthread_mutex_t  mutex; /* mutexTurno */
...
{ 
	...
	*/
	...
	while(1) {
		...
		DBGpthread_mutex_lock( &mutex, Plabel); /* faccio la lock cosi da controllare di avere il ponte free */
		autoInAttesa[indiceSenso]++;
		while(biglietto[indiceSenso] != myBiglietto /* Se non è il mio turno */
			|| ponteLibero == 1 /* se il ponte è pieno */
			|| autoInAttesa[indiceSenso] < autoInAttesa[!indiceSenso] /* se la mia coda non è la piu affollata */
			|| (autoInAttesa[indiceSenso] == autoInAttesa[!indiceSenso] && indiceSenso != INDICESENSOORARIO)) /* se le due code sono pari e non sono orario */
		{
			DBGpthread_cond_wait(&condPonteLibero, &mutex, Plabel);
		}
		autoInAttesa[indiceSenso]--;
		ponteLibero = 1; /* ponte occupato */
		...
		ponteLibero = 0; /* ponte liberato */
		biglietto[indiceSenso]++; /* incremento il turno corrente per la mia fila */
